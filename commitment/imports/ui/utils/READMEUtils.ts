...
export const markdownToHtml = (markdown: string): string =>
  markdown
    ...
    .replace(
      /^### (.*$)/gim,
      '<h3 class="text-lg font-semibold text-git-text-primary mb-2">$1</h3>'
    )
    ...
    .replace(
      /```([\s\S]*?)```/g,
      '<pre class="bg-git-bg-secondary border border-git-stroke-secondary rounded-lg p-4 my-4 overflow-x-auto"><code class="text-git-text-secondary text-sm">$1</code></pre>'
    )

    // Inline code
    .replace(
      /`([^`]+)`/g,
      '<code class="bg-git-bg-primary px-1 rounded text-git-text-primary text-sm">$1</code>'
    )
    ...
    .replace(
      /\[([^\]]+)\]\(([^)]+)\)/g,
      '<a href="$2" target="_blank" rel="noopener noreferrer" class="text-git-accent-primary hover:text-git-accent-primary-hover underline">$1</a>'
    )
    ...
    .replace(
      /(<li.*<\/li>)/gim,
      '<ul class="list-disc list-inside text-git-text-secondary space-y-1 my-2">$1</ul>'
    )
