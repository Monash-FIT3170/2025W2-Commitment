import {
  ContributorMetrics,
  ContributorScaledData,
  ContributorScaledMetric,
  FilteredData,
  MetricType,
  SerialisableMapObject,
  ContributorData,
  RepoMetricDistribution,
...
export function getContributorMetrics(data: FilteredData): ContributorMetrics[] {
  ...
  return contributors.map((contributor) => ({
    ...
    metrics: {
      [MetricType.TOTAL_COMMITS]: getTotalCommitsPerContributor(
        repositoryData,
        contributor.value.name
      ),
      [MetricType.LOC]: getLOCperContributor(repositoryData, contributor.value.name),
      [MetricType.LOC_PER_COMMIT]: getLocPerCommitPerContributor(
        repositoryData,
        contributor.value.name
      ),
      [MetricType.COMMITS_PER_DAY]: getCommitPerDayPerContributor(
        repositoryData,
        contributor.value.name
      ),
    },
 ...
 */
export function getContributorScaledMetric(
  passedContributor: SerialisableMapObject<string, ContributorData>,
  data: FilteredData,
  ...
  const contributorMetric = contributorMetrics.find(
 ...
 */
export function getContributorScaledData(
  data: FilteredData,
  selectedMetric: MetricType
): ContributorScaledData[] {
  const {
 ...
 */
export function getRepoMetricDistribution(
  data: FilteredData,
  selectedMetric: MetricType
): RepoMetricDistribution {
  // get all contributor metrics
  ...
  const mean = roundToTwoDecimal(
    metricValues.reduce((acc, val) => acc + val, 0) / metricValues.length
  );
  const median = roundToTwoDecimal(
    metricValues.length % 2 === 0
 ...
 */
export function getScalingDistributionResult(
  data: FilteredData,
  selectedMetric: MetricType
): ScalingDistributionResult {
